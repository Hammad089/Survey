...
export default function NotStarted({AssignmentData, navigation}) {
  ...
  return (
    ...
    ongoingAssignment.length > 0 ? (ongoingAssignment.map((item, index)=> {
        ...
        <TouchableOpacity onPress={() => navigation.navigate('circle', {ItemId : item.assignment_id, AssignmentModifiedID:item.assignment_modified_id, AssignmentsID:item.assignment_id})}>
        ...
        <View style={{flexDirection:'row', justifyContent:'space-between', marginTop:10}}>
            ...
            <View>
                <Text>Published:<Text style={{ color: 'red', marginBottom: 15, fontSize: 14, fontWeight:'500' }}></Text></Text>
            </View>
        </View>
      </View>
          )
